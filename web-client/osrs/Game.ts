...
import { MIDIPlayer } from "./sound/MidiPlayer/MIDIPlayer";
...
export class Game extends GameShell {
    ...
    public adjustMidiVolume$boolean$byte$int(flag: boolean, byte0: number, volume: number) {
        ...
        if (flag) {
            ...
            // -1200=min volume, 0=max volume, 1300 so that 1 = 10
    ...
    public async startUp() {
        ...
        // this.onDemandRequester.request(2, 0);
        // console.log(this.onDemandRequester)
        // throw Error("HERE")
    ...
    public processOnDemandQueue(flag: boolean) {
        if (flag) {
            this.opcode = -1;
        }
        do {
    ...
    async initStores() {
        const promises = await Promise.all([
            this.getFile(cacheData.client_cache["main_file_cache.dat"]),
            this.getFile(cacheIndices.client_cache["main_file_cache.idx0"]),
            this.getFile(cacheIndices.client_cache["main_file_cache.idx1"]),
            this.getFile(cacheIndices.client_cache["main_file_cache.idx2"]),
            this.getFile(cacheIndices.client_cache["main_file_cache.idx3"]),
            this.getFile(cacheIndices.client_cache["main_file_cache.idx4"])
        ]);
    ...
    public processMenuActions(id: number) {
        ...
        if (action === 100) {
            ...
            if (!flag4) {
                flag4 = this.walk(false, false, second, Game.local.pathY[0], 1, 1, 2, 0, first, 0, 0, Game.localPlayer.pathX[0]);
            }
            this.anInt1020 = this.clickX;
            this.anInt1021 = this.clickY;
            this.crossType = 2;
