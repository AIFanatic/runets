export interface ServerItemRequirements {
    skillId: number;
    level: number;
};

export interface ServerItemBonuses {
    attack_stab: number;
    attack_slash: number;
    attack_crush: number;
    attack_magic: number;
...
export interface ServerItemDefinition {
    ...
    attack_speed: number;
    absorb: string; // check
    bonuses: ServerItemBonuses; // check
    ...
    requirements: ServerItemRequirements[]; // check {0,30}-{4,30}-{6,30} {skillid,level}
    weight: number;
    ge_buy_limit: number;
    bankable: boolean;
    rare_item: boolean;
